...
describe("PriToken.sol", async () => {
  ...
  it("holder should be able to transfer", async () => {
    ...
    await expect(from.token.transfer(to.address, amount)).to.emit(
      token,
      "Transfer"
    );
  ...
  it("holder should be able to approve", async () => {
    ...
    const amount = ethers.utils.parseEther("200");
    await expect(approver.token.approve(spender.address, amount)).to.emit(
      token,
      "Approval"
    );
    const allowance = await approver.token.allowance(
      approver.address,
      spender.address
    );
  ...
  it("approved should be able to transfer", async () => {
    ...
    const spender = users[1];
    const amount = ethers.utils.parseEther("200");
    const approverBalance = BigNumber.from(
      await token.balanceOf(approver.address)
    );
    const spenderBalance = BigNumber.from(
      await token.balanceOf(spender.address)
    );
    await approver.token.approve(spender.address, amount);
    await expect(
      spender.token.transferFrom(approver.address, spender.address, amount)
    ).to.emit(token, "Transfer");
    const approverBalance_new = BigNumber.from(
      await token.balanceOf(approver.address)
    );
    const spenderBalance_new = BigNumber.from(
      await token.balanceOf(spender.address)
    );
